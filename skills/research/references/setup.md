...
- **CLI + Skills** — installs a `research` skill that guides the agent to use `ctx7` CLI commands (no MCP required)
...
- A `research` skill in the agent's skills directory

**What gets written — CLI + Skills mode:**

- A `research` skill in the chosen agent's skills directory, guiding the agent to use `ctx7 library` and `ctx7 docs` commands

## See Also

- **[Documentation Lookup](docs.md)** — Query docs after setup is complete
- **[Skills Management](skills.md)** — Install and manage skills after MCP/CLI is configured
- **[Main Skill](../SKILL.md)** — Full workflow overview and troubleshooting guide
